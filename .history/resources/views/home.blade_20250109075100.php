        ...
        <div id="notification-container" class="space-y-4">
            <div class="bg-white shadow-lg rounded-lg p-6">
                ...
                <div class="mt-4" id="notifications">
                    ...
                    <!-- Dummy Data -->
                    <div class="bg-green-100 text-green-800 p-3 rounded-md mb-2">BRT ONE 20 BLUME COIN RECEIVED</div>
                    <div class="bg-yellow-100 text-yellow-800 p-3 rounded-md mb-2">BRT ALPINE 50 BLUME COIN UPDATED</div>
                    <div class="bg-red-100 text-red-800 p-3 rounded-md mb-2">BRT TWO 100 BLUME COIN DELETED</div>
                    <div class="bg-green-100 text-green-800 p-3 rounded-md mb-2">BRT ONE 20 BLUME COIN RECEIVED</div>
                    <div class="bg-yellow-100 text-yellow-800 p-3 rounded-md mb-2">BRT ALPINE 50 BLUME COIN UPDATED</div>
                    <div class="bg-red-100 text-red-800 p-3 rounded-md mb-2">BRT TWO 100 BLUME COIN DELETED</div>
                    <div class="bg-green-100 text-green-800 p-3 rounded-md mb-2">BRT ONE 20 BLUME COIN RECEIVED</div>
...
<script>
    ...
    channel.bind('brt-updated', function(event) {
        ...
        if (event.brt.status === 'created') {
            ...
            newNotification.textContent = `BRT ONE ${event.brt.amount} BLUME COIN RECEIVED`;
        } else if (event.brt.status === 'updated') {
            newNotification.classList.add('bg-yellow-100', 'text-yellow-800', 'p-3', 'rounded-md', 'mb-2');
            newNotification.textContent = `BRT ALPINE ${event.brt.amount} BLUME COIN UPDATED`;
        } else if (event.brt.status === 'deleted') {
            newNotification.classList.add('bg-red-100', 'text-red-800', 'p-3', 'rounded-md', 'mb-2');
            newNotification.textContent = `BRT TWO ${event.brt.amount} BLUME COIN DELETED`;
        }
