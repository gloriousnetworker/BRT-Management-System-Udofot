        ...
        <div class="bg-white shadow-lg rounded-lg p-6">
            ...
            <div class="mt-4">
                <ul id="ticket-list" class="space-y-4">
                    ...
                    <li class="flex justify-between items-center bg-gray-100 p-4 rounded-lg" data-id="{{ $ticket->id }}">
                        <div>
                            <div class="text-lg font-semibold">BRT Code: {{ $ticket->brt_code }}</div>
                            <div class="text-sm text-gray-600">Reserved Amount: {{ $ticket->reserved_amount }} BLUME COINS</div>
                            <div class="text-sm text-gray-600">Status: {{ ucfirst($ticket->status) }}</div>
                        </div>
                        <div class="flex space-x-2">
                            <button class="bg-yellow-500 text-white px-3 py-2 rounded-md update-ticket" data-id="{{ $ticket->id }}">Update</button>
                            <form action="{{ route('tickets.destroy', $ticket->id) }}" method="POST" class="delete-ticket-form">
                                @csrf
                                @method('DELETE')
            ...
            <div class="bg-white shadow-lg rounded-lg p-6">
                ...
                <div class="mt-4" id="notifications">
                    ...
                    <div class="bg-green-100 text-green-800 p-3 rounded-md mb-2">
                        ...
                        <span class="text-sm text-gray-600 ml-2">{{ $notification->created_at->diffForHumans() }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            if (response.ok) {
                ...
                <li class="flex justify-between items-center bg-gray-100 p-4 rounded-lg">
                    <div>
                        <div class="text-lg font-semibold">BRT Code: ${ticket.brt_code}</div>
                        <div class="text-sm text-gray-600">Reserved Amount: ${ticket.reserved_amount} BLUME COINS</div>
                        <div class="text-sm text-gray-600">Status: ${ticket.status.charAt(0).toUpperCase() + ticket.status.slice(1)}</div>
                    </div>
                    <div class="flex space-x-2">
                        <button class="bg-yellow-500 text-white px-3 py-2 rounded-md update-ticket" data-id="${ticket.id}">Update</button>
                        <form action="/tickets/${ticket.id}" method="POST" class="delete-ticket-form">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500 text-white px-3 py-2 rounded-md">Delete</button>
                        </form>
