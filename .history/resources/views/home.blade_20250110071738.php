...
<script>
    ...
    // Initialize Pusher with the necessary keys and cluster
    ...
    // Bind the 'TicketCreated' event to a callback function
    channel.bind('TicketCreated', function(data) {
        // Show an alert when a new ticket is created
        alert(`New ticket created: ${data.brt_code} with amount $${data.reserved_amount}`);

        // Optionally, you can update the UI to reflect the new ticket
        const ticketsSection = document.getElementById('tickets-section'); // Make sure you have a section for tickets
        const newTicketHTML = `
            <div class="bg-green-100 p-4 rounded-lg flex justify-between items-center">
                <div>
                    <div><strong>BRT Code:</strong> ${data.brt_code}</div>
                    <div><strong>Amount:</strong> $${data.reserved_amount}</div>
                    <div><strong>Status:</strong> Active</div>
                </div>
            </div>
        `;
        ticketsSection.innerHTML += newTicketHTML; // Append the new ticket
    });

    // Trigger the 'TicketCreated' event when the ticket is created
    document.getElementById('create-ticket-btn').addEventListener('click', function(e) {
        // Prevent default form submission behavior
        e.preventDefault();

        // Get form data
        var brtCode = document.getElementById('brt_code').value;
        var reservedAmount = document.getElementById('reserved_amount').value;

        // Trigger the Pusher event
        pusher.trigger('tickets', 'TicketCreated', {
            brt_code: brtCode,
            reserved_amount: reservedAmount
        });

        // You can also submit the form here after triggering the event (optional)
