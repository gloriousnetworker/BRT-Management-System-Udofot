...
Route::post('/tickets', function (Illuminate\Http\Request $request) {
    ...
    $ticketName = $request->input('name'); // Example field for ticket name

    // Trigger the event
    event(new TicketCreated($ticketName));

    return response()->json(['message' => 'Ticket created and event triggered'], 201);
})->name('tickets.store');
